...
LOCK_FILE="/tmp/setup-mediawiki.lock"
...
function setup() {
    ...
    if [[ $(echo "$pmaEnable" | $(which tr) '[:lower:]' '[:upper:]') = "TRUE" ]]; then
        ...
        if [[ ! -f docker-compose.yml ]]; then
            $(which wget) -O docker-compose.yml https://raw.githubusercontent.com/mindevis/scripts/main/composes/mediawiki/docker-compose-mediawiki-mysql-without-phpmyadmin.yml
        fi
    fi

    if [[ ! -f manage.sh ]]; then
        $(which wget) -O manage.sh https://raw.githubusercontent.com/mindevis/scripts/main/composes/mediawiki/scripts/manage.sh
