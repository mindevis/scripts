...
function stop() {
    echo -e "${WARN}Stop docusaurus environment.${NORMAL}"
    $(which docker) compose stop
    exit 0
}

function restart() {
    echo -e "${WARN}Restart docusaurus environment.${NORMAL}"
    $(which docker) compose restart
    exit 0
}

function destroy() {
    echo -e "${WARN}Destroy docusaurus environment.${NORMAL}"
    $(which docker) compose -f docker-compose.yml down
    $(which docker) volume rm --force docusaurus-data
    $(which docker) volume prune --force
    $(which docker) rmi docusaurus
    exit 0
}
